...
class tm_score_embeds_dataset(Dataset):
    ...
    def __init__(self, hdf_file, tm_pairs):
        ...
        # load pairs
        self.tm_score_emb_data = pd.read_csv(tm_pairs, sep="\t", header=None, low_memory=False)
    ...
    def __getitem__(self, idx):
        ...
        with h5py.File(self.hdf_file, 'r') as f:
            _, idx1, idx2, tm_score = self.tm_score_emb_data.loc[idx].values
...
def construct_datasets(hdf_file, tm_pairs, train_prop=.9, val_prop=.05, test_prop = .05):
    total_samples = len(tm_score_embeds_dataset(hdf_file=hdf_file, tm_pairs=tm_pairs))
    ...
    train_ds = torch.utils.data.Subset(tm_score_embeds_dataset(hdf_file=hdf_file, tm_pairs=tm_pairs), train_indices)
    val_ds =  torch.utils.data.Subset(tm_score_embeds_dataset(hdf_file=hdf_file, tm_pairs=tm_pairs), val_indices)
    test_ds = torch.utils.data.Subset(tm_score_embeds_dataset(hdf_file=hdf_file, tm_pairs=tm_pairs), test_indices)
